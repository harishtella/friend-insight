package flare.controls
{
	 ...
	 */
	public class TooltipControl extends flare.vis.controls.Control
	{		
		...
		private var tooltips:Vector.<String> = new Vector.<String>();
		...
		{			
			this.filter = (filter==null) ? findTooltip : filter;
		...
		public function addTooltip(n:DisplayObject, tip:String):void {
			objects.push(n);
			tooltips.push(tip);	
		}
		...
		private function findTooltip(n:DisplayObject):Boolean
		{
			for (var i:int = 0; i<objects.length; i++) {
				if (n==objects[i]) {
					TextSprite(tooltip).htmlText = tooltips[i];
					return true;
				}
			}			
			return false;
